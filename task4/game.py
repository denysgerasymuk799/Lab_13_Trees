...
def start_game():
    ...
    while len(empty_cells(game_board.board)) > 0 and\
        ...
        if first == 'N':
            game_board.computer_step(h_choice, c_choice, game_step)
            first = ''

        game_step += 1

        game_board.human_step(h_choice, c_choice)
        game_board.computer_step(h_choice, c_choice, game_step)
        game_step += 1

    # Game over message
    if game_board.wins(game_board.board, HUMAN):
        clean()
        print(f'Human turn [{h_choice}]')
        game_board.display(game_board.board, h_choice, c_choice)
        print('My congritulations - YOU WIN!')
    elif game_board.wins(game_board.board, COMPUTER):
        clean()
